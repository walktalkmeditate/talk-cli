...
>
> **Decision superseded (2026-06-09, during Plan 2 audio verification): use direct HF URLs.** Option A's premise was false — **GitHub release assets 302-redirect too** (to `release-assets.githubusercontent.com`), so `redirects(0)` could never fetch *any* asset. Plan 2's `download::fetch` now follows redirects manually (every hop must be absolute `https://`, case-insensitive scheme, 5-hop cap, 1 GiB `read_capped` before the hash gate; adversarially reviewed, 0 confirmed findings). That is exactly the alternative above, so HF `resolve/main` URLs work directly and the solo maintainer never re-hosts 491 MB. The pinned SHA-256 stays the integrity guarantee; the GGUF pin is additionally corroborated by HF's LFS oid record.
...
/// (~491 MB) + its tokenizer.json (a separate asset, not bundled in the GGUF).
/// Direct HF URLs (fetch follows https-only redirects); hashes pinned 2026-06-09
/// from the real assets — the GGUF pin matches HF's LFS oid record.
...
**T1 spike RECORDED (2026-06-09, Apple Silicon arm64, release build, candle 0.9.2):** cold load **749 ms**; per-phrase **p50 1574 ms · p95 1754 ms** (5 phrases, argmax, ≤96 new tokens) — **~7× over the ~250 ms realtime window on the *fast* hardware floor** (x86 unmeasured, a fortiori worse). Quality at 0.5B-Q4-greedy: 2/5 outputs were meaning-edits the guard correctly rejects ("trying to say"→"want to say"; dropped "there's"), 1/5 came back **ALL CAPS** (which the case-insensitive content-word guard would *accept*), 2/5 clean. Also found: the `formatter` dep graph needs **rustc ≥ 1.83** (`zip` via candle-core declares 1.83); default/listen/download builds verified holding MSRV 1.82 with the MSRV-aware resolver. **Decision gate outcome: the realtime per-phrase async swap (T9 as drafted) is a no-go on the evidence** — both latency (7× over) and marginal quality vs deterministic-Light. The shipping shape for the LLM (post-session polish vs descope-to-later-model) is the recorded T1 fork, resolved by the user — see the session log / build-state memory.

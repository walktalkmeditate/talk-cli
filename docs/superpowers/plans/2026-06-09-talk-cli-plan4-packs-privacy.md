...
Implementation: `State` gains `disclosed: bool` (serde default false). In `main()` after config/base resolution, for session commands (Journal/Reflect/bare — NOT unburden/config/thread/streak/download): load state; if `!disclosed`, print the note to stderr, set + save state (0600 via `write_private`). Works identically in `--from-text` and live paths (it precedes the session). Integration test: first journal run's stderr contains "local"; second run's doesn't.
